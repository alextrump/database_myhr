...
void HR_department::showStaffTable()
{
    ...
    QString queryText = QString(
            "select * from \"kurylenko\".show_staff "
            "where date_in <= '%1' "
...
void HR_department::on_button_printReport_clicked()
{
    ...
    out << "<html>"
       ...
       << QString("<h2 align=center>%1</h2>").arg("Звіт співробітника підприємства")
       << "<br/>";
    ...
    queryText = QString("select * from \"kurylenko\".person where \"Id_person\" = %1;").arg(persId);
    ...
    queryText = QString(
            ...
            "from \"kurylenko\".personal_skills per_skills "
                "join \"kurylenko\".skills "
    ...
    out << QString("<h4>%1</h4>").arg("Навички:")
    ...
    out << QString("<h4>%1</h4>").arg("Попередні посади:")
        << "<table border = 1 cellspacing=0 cellpadding=2 width=\"100%\">"
        << "<thead><tr bgcolo=#f0f0f0>";
    for( int column = 0; column < columnCount; column++)
...
{
    QString queryText =
            "select * from \"kurylenko\".show_vacancies;";
    QSqlQueryModel *model = db->getQueryModel(queryText);
...
{
    QString queryText =
            "select * from \"kurylenko\".show_positions;";
    QSqlQueryModel *model = db->getQueryModel(queryText);

    model->setHeaderData(0, Qt::Horizontal, tr("Посада"));
    model->setHeaderData(1, Qt::Horizontal, tr("Кількість співробітників"));
...
void HR_department::on_button_deletePosition_clicked()
{
    ...
    QString queryText = QString("select \"kurylenko\".delete_position(%1);").arg(selectedPositId);
