...
void Dialog_editPosition::showInpValues()
{
    ...
    queryText = QString(
        "select * from \"kurylenko\".positions where \"Id_position\" = %1;").arg(positId);
    ...
    queryText = QString(
                ...
                "from \"kurylenko\".neccessary_skills nec_skills "
                    "join \"kurylenko\".skills skills "
...
void Dialog_editPosition::accept()
{
    ...
    {
        queryText = QString(
            "insert into \"kurylenko\".positions (posit_name, posit_description, exists) values ('%1', '%2', TRUE);").arg(name).arg(description);
        ...
        queryText = QString(
            "delete from \"kurylenko\".necessary_skills "
            "where \"Id_position\" = %1;").arg(positId);
        execPipeResultStatus = execPipeResultStatus && db->executeQuery(queryText, "admin", this, -1);

        queryText = QString(
            "update \"kurylenko\".positions set "
                "posit_name = '%1', "
                "posit_description = '%2' "
    ...
    {
        queryText = QString(
            "insert into \"kurylenko\".necessary_skills (\"Id_skill\", \"Id_position\") "
