...
bool DB_setup::connect_to_db(QWidget* qwidget)
{
    ...
    base.setPort(5432);
    base.setDatabaseName("myhr");
    base.setUserName(usercool);
    base.setPassword(passwordcool);
...
bool DB_setup::executeQuery(QString queryText, QString executer, QWidget* qwidget, int queryType)
 {
     ...
     if (!executed){
         QString errmess = query.lastError().databaseText();
         int inderr = errmess.indexOf('.', 0);
         showMessage("Помилка", errmess.mid(0, inderr+1));
     } else {
