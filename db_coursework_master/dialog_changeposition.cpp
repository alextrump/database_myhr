...
void Dialog_changePosition::showInpValues()
{
    ...
    queryText = QString("select * from \"kurylenko\".show_shortStaffInfo where \"Id_staff\" = %1;").arg(staffId);
...
void Dialog_changePosition::updatePositSelect()
{
    ...
    queryText = "select * from \"kurylenko\".show_vacancies;";
...
void Dialog_changePosition::accept()
{
    ...
    QString queryText = QString("select \"kurylenko\".change_staff_position(%1, %2, %3, '%4');").arg(persId).arg(staffId).arg(selectedPositId).arg(changeDate);
